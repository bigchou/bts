...
def test(params):
    ...
    model = BtsModel(params=args).to('cpu')




    #'''
    dummy_input = torch.rand([1, 3, 352, 1216]).to(torch.float32)
    #dummy_focal = torch.rand([1]).to(torch.float32)
    torch.onnx.export(
        model, (dummy_input),#(dummy_input, dummy_focal),
        'bts.onnx',
        input_names=['input'],#input_names=['input', 'focal'],
        export_params=True,
    ...
    with torch.no_grad():
        for _, sample in enumerate(tqdm(dataloader.data)):
            ...
            # focal = tensor([721.5377], device='cuda:0', dtype=torch.float64)
            #pdb.set_trace()
            # Predict
